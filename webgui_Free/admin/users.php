...
<tr>
		<?php 
			...
			while ($getInfo = $SQLGetUsers -> fetch(PDO::FETCH_ASSOC))
			{
				...
				$email = $getInfo['email'];
				$rank = $getInfo['rank'];
$status = ($getInfo['status'] == 0) ? 'Active' : 'Banned';
				...
				echo '<tr class="gradeX"><td>'.$user.'</td><td>'.$email.'</td><td>'.$rank.'</td><td>'.$status.'</td><td width="50px"><a href="edit.php?id='.$id.'"><button class="btn btn-info">Edit</button></a></td></tr>';
			}
			?>
</tbody>
</table>
